...
export class ClientComponent implements OnInit {
  ...
  /**
   * Cria um link de telefone válido removendo caracteres não numéricos
   * @param phoneNumber Número de telefone
   * @returns Link de telefone formatado
   */
  getPhoneLink(phoneNumber: string | null | undefined): string {
    if (!phoneNumber) {
      return 'tel:';
    }

    // Retorna o link com apenas os dígitos
    return 'tel:' + phoneNumber.replace(/\D/g, '');
  }

  /**
   * Formata o número de telefone para o formato (XX) XXXXX-XXXX ou (XX) XXXX-XXXX
  ...
  formatPhoneNumber(phoneNumber: string | null | undefined): string {
    ...
    // Verifica se é um número de telefone válido (10 ou 11 dígitos)
    if (cleaned.length !== 10 && cleaned.length !== 11) {
      // Se não for um formato válido, retorna o número original mas limita o tamanho para evitar problemas de layout
      return phoneNumber.length > 15 ? phoneNumber.substring(0, 15) + '...' : phoneNumber;
    }
    ...
    // Formata como (XX) XXXX-XXXX para telefone fixo (10 dígitos)
  ...
  protected fillComponentAttributesFromResponseBody(data: IClient[] | null): IClient[] {
    ...
    // Garante que todos os objetos de cliente tenham as propriedades necessárias
    return data.map(client => ({
      ...client,
      // Garante que propriedades opcionais tenham valores padrão quando nulas
